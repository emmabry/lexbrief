...
def llama_summary(text, model_name="llama3"):
    ...
    prompt = (
    f'''You are a summarisation engine for official EU legal and policy documents. Your task is to generate detailed, factual summaries that clearly convey the document’s purpose, obligations, scope, and legal structure.
...
- Write the summary in clear, well-structured paragraphs rather than bullet points.
- Provide a detailed and comprehensive summary that thoroughly covers the document's key points.
- If the input text is longer, produce a correspondingly longer and more detailed summary.
- Include the document's purpose, legal obligations, provisions, exemptions, scope, and effective date if stated.
- Use neutral, formal language suitable for non-expert policy readers.
- Do not overly shorten or omit key details to keep the summary concise.
...
if __name__ == "__main__":
    ...
    source_docs = source_docs[50:100]
    target_summaries = target_summaries[50:100]
    generated_summaries = []

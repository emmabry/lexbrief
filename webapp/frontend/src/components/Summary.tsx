...
function Summary({ summaryData, celexData, celexId }: SummaryProps) {
  const [showAllModifies, setShowAllModifies] = useState(false);
  const [showAllModifiedBy, setShowAllModifiedBy] = useState(false);

  const modifiesItems = celexData?.related_documents.modifies || [];
  const modifiedByItems = celexData?.related_documents.modified_by || [];

  const displayModifies = showAllModifies ? modifiesItems : modifiesItems.slice(0, 3);
  const displayModifiedBy = showAllModifiedBy ? modifiedByItems : modifiedByItems.slice(0, 3);
    ...
    <div className='d-flex flex-row align-items-start px-5 w-100'>
      <div className="m-4" style={{ width: '60%' }}>
        <div className="summary-l-card border p-4 m-3 rounded-3">
          ...
          <button
            className="btn my-button mt-3 w-100 d-flex justify-content-center align-items-center"
            onClick={() =>
              window.open(
                `https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A${celexId}&qid=1754333530919#PP4Contents`,
                '_blank'
              )
            }
          >
          ...
          {((modifiesItems.length ?? 0) > 0 || (modifiedByItems.length ?? 0) > 0) && (
            <div className="border-top">

              {modifiesItems.length > 0 && (
                <div>
                  <h6 className="mt-4">
                    <LinkIcon className="link-icon" />Acts affected by this legislation
                  </h6>
                  {displayModifies.map((item, idx) => (
                    <div key={idx} className="border reference-box p-2 rounded-3 mb-2">
                      ...
                      <a
                        ...
                        aria-label={`Open ${item.Act.celex} in a new tab`}
                        className="mb-0"
                      >
                        <PopupIcon className="link-icon" />
                      </a>
                    </div>
                  ))}
                  {modifiesItems.length > 3 && (
                    <div className="d-flex justify-content-center mt-2">
                      <button
                      className="btn my-button w-100 d-flex justify-content-center align-items-center"
                      onClick={() => setShowAllModifies(!showAllModifies)}
                      >
                        {showAllModifies ? "Show Less" : "Show More"}
                      </button>
                    </div>
                  )}
                </div>
              ...
              {modifiedByItems.length > 0 && (
                <div>
                  <h6 className="mt-4">
                    <LinkIcon className="link-icon" />Acts that affect this legislation
                  </h6>
                  {displayModifiedBy.map((item, idx) => (
                    <div key={idx} className="border reference-box p-2 rounded-3 mb-2">
                      <div className="d-flex">
                        <p className="rounded-5 border small fw-semibold px-2 mb-0 me-2">{item.Act.celex}</p>
                  ...
                  {modifiesItems.length > 3 && (
                    <div className="d-flex justify-content-center mt-2">
                      <button
                      className="btn my-button w-100 d-flex justify-content-center align-items-center"
                      onClick={() => setShowAllModifiedBy(!showAllModifiedBy)}
                      >
                        {showAllModifiedBy ? "Show Less" : "Show More"}
                      </button>
                    </div>
                  )}
                </div>
